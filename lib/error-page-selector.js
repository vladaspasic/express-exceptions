...
ErrorPageSelector.prototype.render = function(error, request, response, next) {
	if(error.statusCode) {
		...
		var message = fmt('An %s occured while processing ["%s"]: %r', name, request.path, error);

		log(this.options.logger, error.logLevel, message);
	}

	if(this.isProductionMode()) {
		var page = this.get(error) || this.options.defaultErrorPage;
